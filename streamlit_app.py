...
# ==================== PAGE: TEST CASES ====================
    ...
    if st.session_state.project_data['test_cases']:
        ...
        if status_counts:
            col_list = st.columns(len(status_counts))
            colors = {'Pending': '🟡', 'In Progress': '🔵', 'Passed': '🟢', 'Failed': '🔴', 'Blocked': '⚫'}
            
            for col, (status, count) in zip(col_list, status_counts.items()):
                with col:
...
# ==================== PAGE: ISSUES TRACKER ====================
    ...
    if st.session_state.project_data['issues']:
        ...
        if severity_counts:
            col_list = st.columns(len(severity_counts))
            colors = {'Low': '🟢', 'Medium': '🟡', 'High': '🔴', 'Critical': '⚫'}
            
            for col, (severity, count) in zip(col_list, severity_counts.items()):
                with col:
